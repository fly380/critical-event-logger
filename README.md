...
- [Підтримка та внесок](#підтримка-та-внесок)
- [Зміни](#зміни)
- [Ліцензія](#ліцензія)
...
- Рекомендується **оновити версію в заголовку плагіна** (`Version: X.Y.Z`) та у `readme.txt` (`Stable tag: X.Y.Z`), потім зібрати ZIP і встановити як звичайне оновлення.
- Для GitHub — створіть **анотований тег** та реліз:
  ```bash
...
> Додайте зображення у `docs/` або оновіть шляхи на власні.

---

## Дорожня карта

- Перенесення логів у `uploads/` за замовчуванням.
- Розширені фільтри (користувацькі поля, URI‑патерни).
- Експорт/імпорт налаштувань.
- Більше backends для AI‑аналізу.

---

## Підтримка та внесок

PR та issues вітаються! Будь ласка, дотримуйтеся принципів:
- Окремий бранч на фічу/виправлення.
- Зміни супроводжуйте коротким описом та, за можливості, тест‑кейсами.
- Для значних змін — відкрийте **issue** для обговорення.
